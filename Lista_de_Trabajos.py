...
class listaDeTrabajos():
    ...
    def __init__(self,window):
        ...
        #Creando el treeview
        self.tabla = ttk.Treeview(wrapper1, columns=(1,2,3,4,5,6,7,8), show="headings")
        ...
        self.tabla.heading(6, text="Seña")
        self.tabla.heading(7, text="Saldo")
        self.tabla.heading(8, text="Id")
        ...
        self.lbl1 = Label(wrapper3, text="Estado")
        ...
        self.lbl1 = Label(wrapper3, text="P / EP / L / E")
        ...
        self.ent1= Entry(wrapper3, textvariable=self.t2)
        ...
        self.ent2= Entry(wrapper3, textvariable=self.t3)
        self.ent2.grid(row=3, column=1, pady=3, columnspan=3, sticky= W+E)
        
        self.lbl3 = Label(wrapper3, text="Precio")
        self.lbl3.grid(row=4, column=0, padx=5, pady=3)
        self.ent3= Entry(wrapper3, textvariable=self.t4)
        self.ent3.grid(row=4, column=1, pady=3, columnspan=3, sticky= W+E)
        
        self.lbl3 = Label(wrapper3, text="Seña")
        self.lbl3.grid(row=5, column=0, padx=5, pady=3)
        self.ent3= Entry(wrapper3, textvariable=self.t5)
        ...
        self.lim_btn=Button(wrapper3, text="Limpiar", command= self.limpiarc)
...
#---------------------------METODOS------------------------        
    ...
    def search(self):
        q2=self.ent.get()
        query= 'SELECT Estado, Fecha, Cliente, Trabajo, Precio, Seña, Saldo, Id FROM Lista_de_Trabajos WHERE Cliente = ?'
    ...
    def getrow(self, event): #selecciono los datos y los paso a las variables t
        ...
        self.t6.set(item['values'][7])#varible que guarda el ID para eliminar
    ...
    def agregar(self):
        ...
        if  len(estado) != 0 and len(cliente) !=0 and len(trabajo) !=0 and len(precio) !=0  and len(seña) !=0 :  #valido que no le pasen valores vacios
            ...
            query='INSERT INTO Lista_de_Trabajos(Estado, Fecha, Cliente, Trabajo, Precio, Seña, Saldo, Id) VALUES(?,?,?,?,?,?,?,NULL)' 
            self.cursor.execute(query,(estado,fecha,cliente,trabajo,precio,seña,saldo))
            self.conn.commit()
    ...
    def modificar(self):
        ...
        if  len(estado) != 0 and len(cliente) !=0 and len(trabajo) !=0 and len(precio) !=0 and len(seña) !=0:  #valido que no le pasen valores vacios
            ...
            if messagebox.askyesno("Confirmar Modificacion","Estas Seguro de Modificar este trabajo?"):
                query = "UPDATE Lista_de_Trabajos SET Estado=?, Fecha=?, Cliente=?, Trabajo=?, Precio=?, Seña=?, Saldo=?, Id=? WHERE Id= ?" 
                self.cursor.execute(query,(estado,fecha,cliente,trabajo,precio,seña,saldo,id,id))
                self.conn.commit()
                self.limpiar()
            else:
                return True    
